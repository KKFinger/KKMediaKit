...
- (void)setCornerImageWithURL:(NSURL *)url placeholder:(UIImage *)placeholder compressDataSize:(CGFloat)size{
    @weakify(self);
    [self sd_setImageWithURL:url placeholderImage:placeholder completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        ...
        if(size != -1){
            rstImage  = [UIImage imageWithData:[UIImage compressImage:rstImage toByte:size quality:1]];
        }
