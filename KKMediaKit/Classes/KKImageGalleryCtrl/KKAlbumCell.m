            ...
            for(NSInteger i = 0 ; i < 3 ; i++){
                if (i < albumIfo.assetCount){
                    if(type == KKAlbumCellImage){
                        [[KKPhotoManager sharedInstance]getImageWithAlbumID:albumIfo.albumId index:i needImageSize:CGSizeMake(90, 90) isNeedDegraded:NO sort:NSOrderedDescending block:^(KKPhotoInfo *item) {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [self setImage:item.thumbImage atImageIndex:i];
                            });
                        }];
                    }else if(type == KKAlbumCellVideo){
                        [[KKVideoManager sharedInstance]getVideoCorverWithAlbumId:albumIfo.albumId index:i needImageSize:CGSizeMake(90, 90) isNeedDegraded:NO block:^(KKVideoInfo *videoInfo) {
                            dispatch_async(dispatch_get_main_queue(), ^{
