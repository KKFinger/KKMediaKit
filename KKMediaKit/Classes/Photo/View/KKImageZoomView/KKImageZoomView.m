...
- (void)showImageWithUrl:(NSString *)imageUrl placeHolder:(UIImage *)placeholder {
    _imageUrl = imageUrl;
    [[SDImageCache sharedImageCache]queryCacheOperationForKey:self.imageUrl done:^(UIImage * _Nullable image, NSData * _Nullable data, SDImageCacheType cacheType) {
        if(image){
            [self displayImage:image];
        }else{
            [self loadImageWithUrl:self.imageUrl placeHolder:placeholder];
        }
    }];
}
...
- (void)displayImage:(UIImage *)image{
    ...
    if(imageH > self.height){
        self.contentOffset = CGPointMake(self.contentOffset.x, (imageH- self.height) / 2.0);
    }
...
- (void)loadImageWithUrl:(NSString *)url placeHolder:(UIImage *)placeHolder{
    ...
    [self showSysActivityWithStyle:UIActivityIndicatorViewStyleGray];
