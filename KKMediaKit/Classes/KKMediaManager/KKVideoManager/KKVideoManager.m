...
- (void)getVideoCorverWithAlbumId:(NSString *)albumId
                            index:(NSInteger)index
                    needImageSize:(CGSize)size
                   isNeedDegraded:(BOOL)degraded
                            block:(void(^)(KKVideoInfo *videoInfo))handler{
    [self getVideoAlbumCollectionWithAlbumId:albumId block:^(PHAssetCollection *collection){
        if (collection != nil){
            PHFetchOptions *options = [[PHFetchOptions alloc] init];
            options.predicate = [NSPredicate predicateWithFormat:@"mediaType = %d",PHAssetMediaTypeVideo];
            PHFetchResult *assetsResult = [PHAsset fetchAssetsInAssetCollection:collection options:options];
            if (assetsResult && index < assetsResult.count){
